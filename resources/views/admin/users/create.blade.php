        ...
        <div class="glass-card p-6">
            <form action="{{ route('admin.users.store') }}" method="POST">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="password" class="block text-sm font-medium text-gray-700 mb-1">{{ __('Password') }} <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label for="password_confirmation" class="block text-sm font-medium text-gray-700 mb-1">{{ __('Confirm Password') }} <span class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="role" id="role" required
                                ...
                                <option value="market_owner" {{ old('role') === 'market_owner' ? 'selected' : '' }}>{{ __('Market Owner') }}</option>
                                <option value="collector" {{ old('role') === 'collector' ? 'selected' : '' }}>{{ __('staff.roles.collector') }}</option>
                                <option value="shop_owner" {{ old('role') === 'shop_owner' ? 'selected' : '' }}>{{ __('Shop Owner') }}</option>
                            </select>
                            @error('role')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
