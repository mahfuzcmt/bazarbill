<x-app-layout>
    ...
    <div class="max-w-3xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="shop_type" id="shop_type" required
                        ...
                        <option value="electronics" {{ old('shop_type', $shop->shop_type) === 'electronics' ? 'selected' : '' }}>{{ __('shops.type_electronics') }}</option>
                        <option value="jewelry" {{ old('shop_type', $shop->shop_type) === 'jewelry' ? 'selected' : '' }}>{{ __('shops.type_jewelry') }}</option>
                        <option value="pharmacy" {{ old('shop_type', $shop->shop_type) === 'pharmacy' ? 'selected' : '' }}>{{ __('shops.type_pharmacy') }}</option>
                        <option value="other" {{ old('shop_type', $shop->shop_type) === 'other' ? 'selected' : '' }}>{{ __('shops.type_other') }}</option>
                    </select>
                    @error('shop_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <!-- Shop Owner -->
                <div>
                    <label for="shop_owner_id" class="block text-sm font-medium text-gray-700">
                        {{ __('shops.owner') }}
                    </label>
                    <select name="shop_owner_id" id="shop_owner_id"
                        ...
                        <option value="{{ $owner->id }}" {{ old('shop_owner_id', $shop->shop_owner_id) == $owner->id ? 'selected' : '' }}>
                            {{ $owner->getLocalizedName() }} ({{ $owner->phone }})
                        </option>
                        @endforeach
                    </select>
                    @error('shop_owner_id')
        ...
        <!-- Delete Form -->
        <form id="delete-form" action="{{ route('market-owner.shops.destroy', $shop) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
    ...
    <script>
        function confirmDelete() {
            if (confirm('{{ __('shops.confirm_delete') }}')) {
